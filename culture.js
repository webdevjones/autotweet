...
var client = new Twitter({
    ...
    access_token_key: config.CULTURE_ACCESS_TOKEN,
    access_token_secret: config.CULTURE_ACCESS_TOKEN_SECRET
})

const tweet_em = async (feed_url) => {
    const xml = await axios.get(feed_url)
    const oldXml = fs.readFileSync('culture.xml', { encoding: 'utf8', flag: 'r' })
    if (oldXml === xml.data) {
        return
    }
    fs.writeFileSync('culture.xml', xml.data)
...
tweet_em('https://www.newsbusters.org/autotweet/culture')