...
var client = new Twitter({
    ...
    access_token_key: config.CNS_ACCESS_TOKEN,
    access_token_secret: config.CNS_ACCESS_TOKEN_SECRET
})

const tweet_em = async (feed_url) => {
    const xml = await axios.get(feed_url)
    const oldXml = fs.readFileSync('cns.xml', { encoding: 'utf8', flag: 'r' })
    if (oldXml === xml.data) {
        return
    }
    fs.writeFileSync('cns.xml', xml.data)
...
tweet_em('https://cnsnews.com/feed/autotweet/1')