...
var client = new Twitter({
    ...
    access_token_key: config.NB_ACCESS_TOKEN,
    access_token_secret: config.NB_ACCESS_TOKEN_SECRET
})

const tweet_em = async (feed_url) => {
    const xml = await axios.get(feed_url)
    const oldXml = fs.readFileSync('nbxml.xml', { encoding: 'utf8', flag: 'r' })
    if (oldXml === xml.data) {
        return
    }
    fs.writeFileSync('nbxml.xml', xml.data)
